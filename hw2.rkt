;toolbox: let, apply, remove, random, [list-ref lst n], member, [last], filter/map
;
;design patter:
;1) filter possible moves/states
;list of list => state
;
;2) simulate until no move is possible (using log)
;
;state: '(1 4 5)
;dice: '( (1 2) (1 2))
...
(define all-distinct?
  (lambda (lst)
    (cond
      [(empty? lst) #t]
      [else (if (member? (first lst) (rest lst)) #f
...
(define list-minus
  (lambda (lst1 lst2)
    (cond
      ((empty? lst1) lst1) ;CONSIDERED
      ((empty? lst2) lst1)
      ((if (member? (first lst1) lst2) (list-minus (rest lst1) lst2)
           (cons (first lst1) (list-minus (rest lst1) lst2)) )))))
...
(define roll
  (lambda (lst)
    (cond
      ((empty? lst) '()) 
      (else (list? (first lst)) (cons (pick-random (first lst)) (roll (rest lst))))
      ))) 

;**************************************************************
; ** problem 4 ** (10 points)
; A move consists of a list of positive integers in *increasing order*
; indicating that flappers with those numbers should be flipped *down*. so like '(1 4)
...
(define ok-move?
  (lambda (move state)
    (cond
      ((empty? move) #t)
      (else (if (member? (first move) state) (ok-move? (rest move) state) #f )))))
...
(define (map-helper n ls)
  (map (lambda (ls) (cons n ls)) ls) )

(define (all-subsets-aux n ls)
  (if (empty? ls) ls
  (append ls (map-helper n ls))))

(define (all-subsets state)
  (cond
  ((= (length state) 2) (list '() (list (last state)) (list (first state)) state))
  ((> (length state) 2) (all-subsets-aux (first state) (all-subsets (rest state))))
  ((= (length state) 1) (list '() (list (first state))))
  ((empty? state) (list '()))
  (else (#f)) ))

;(all-subsets '(0 1 2 5 8))
...
; where sum is the sum of a dice roll: (sum-of (roll standard-dice))
...
;filter altered to help possible-moves
(define (possible-moves-helper n lst)
  (cond
    [(empty? lst) '()]
    [(equal? n (sum-of(first lst)))
     (cons (first lst)
           (possible-moves-helper n (rest lst)))]
    [else (possible-moves-helper n (rest lst))]))


(define (possible-moves sum state)
  (possible-moves-helper sum (all-subsets state)) )

;map altered to help next-states
(define (next-states-helper proc state state1)
  (if (empty? state1)
      '()
      (cons (proc state (first state1))
            (next-states-helper proc state (rest state1)))))


(define (next-states sum state)
  (next-states-helper list-minus state (possible-moves sum state)))

                               
;reminder of what list-minus is: (list-minus '(3 4 5 7 9) '(5 7))
;**************************************************************
; ** problem 7 ** (10 points)
; Write two procedures:
...
;toolbox: let, apply, remove, random, [list-ref lst n], list* member, [last], filter/map
;get the length of each list element
(define (get-length ls)
  (if (empty? ls) ls
      (cons (length (first ls)) (get-length (rest ls) ))))

;how to get the min length from a list of list
;(apply min (get-length (possible-moves sum state))

;given the min length, filter out lists that is not of that length
(define (choose-short-move-filter min-length lst)
  (cond
    [(empty? lst) '()]
    [(= min-length (length(first lst)))
     (cons (first lst)
           (choose-short-move-filter min-length (rest lst)))]
    [else (choose-short-move-filter min-length (rest lst))]))


(define (choose-short-move sum state)
  (if (equal? (possible-moves sum state) '()) '()  
  (let ((min-length (apply min (get-length (possible-moves sum state)))))
  (pick-random (choose-short-move-filter min-length (possible-moves sum state))))))

;get min length, then filter possible-move using min-length, then choose randomly from this filtered list
...
(define (game-play move-chooser state dice)
  (let* ((sum (sum-of (roll standard-dice)))
         (move (move-chooser sum state))
         (new-state (list-minus state move)))
    (if (equal? move '()) '()
    (cons state (game-play move-chooser new-state dice)))
    ;(display state) (display sum) (display move)
    ))
...
; (many-games 100 choose-random-move choose-random-move) => '(44 2 54) win, tie, loss
...
(define (one-game move-chooser1 move-chooser2)
  (cond
    ((< (sum-of(last (game-play move-chooser1 standard-flappers standard-dice))) (sum-of(last (game-play move-chooser2 standard-flappers standard-dice)))) "win")
    ((> (sum-of(last (game-play move-chooser1 standard-flappers standard-dice))) (sum-of(last (game-play move-chooser2 standard-flappers standard-dice)))) "lose")
    (else "tie")))
    
(define (many-games-tally-er n win tie loss move-chooser1 move-chooser2)
  (if (= n 0) (list win tie loss)
  (cond
    ((equal? (one-game move-chooser1 move-chooser2) "win") (many-games-tally-er (- n 1) (+ 1 win) tie       loss move-chooser1 move-chooser2))
    ((equal? (one-game move-chooser1 move-chooser2) "tie") (many-games-tally-er (- n 1)  win       (+ 1 tie) loss move-chooser1 move-chooser2))
    (else (equal? (one-game move-chooser1 move-chooser2) "lose") (many-games-tally-er (- n 1) win      tie       (+ 1 loss) move-chooser1 move-chooser2))))
    ; (display win) ;(display tie) (display loss)
  )

(define (many-games n move-chooser1 move-chooser2)
  (many-games-tally-er n 0 0 0 move-chooser1 move-chooser2))
